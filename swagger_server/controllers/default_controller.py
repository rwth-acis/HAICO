...
def station_information(json_input: dict) -> Tuple[dict, int]:
    ...
    if intent == "station_responsible":
        ...
        code_title, message_title = query.get_station_role(station_id, "Station")
        if code_title == 2:
        ...
        if (
            code_cert != 2
            and code_dataset != 2
            and code_description != 2
            and code_location != 2
            and code_owner != 2
            and code_responsible != 2
            and code_rights != 2
            and code_title != 2
        ):
            return {
                text: f"No information for station {station_id} found.",
                closeContext: true,
            }, 200
    else:
...
def train_information(json_input: dict) -> Tuple[dict, int]:
    ...
    if intent == "train_model":
        ...
        code_creator, message_creator = query.get_train_role(train_id, "creator")
        if code_creator == 2:
            message += message_creator
        code_publisher, message_publisher = query.get_train_role(train_id, "publisher")
        if code_publisher == 2:
            message += message_publisher
        # code_source, message_source = query.
        # if code_creator == 2:
        #     message += message_source
        code_description, message_description = query.get_description(train_id, "Train")
        if code_description == 2:
            message += message_description
        ...
        if (
            ...
            and code_description != 2
            and code_model != 2
            and code_publisher != 2
            and code_title != 2
            and code_version != 2
        ):  # and code_source != 2 and code_pubdate != 2
            return {
                text: f"No information for train {train_id} found.",
                closeContext: true,
            }, 200
    else:
...
def get_performance(json_input: dict) -> Tuple[dict, int]:
    ...
    return {
        text: "Something went wrong: Intent not recognized",
        closeContext: true,
    }, 200
