...
def get_user_info(user_id: str) -> Tuple[int, str]:
    ...
    for item in response["results"]["bindings"]:
        ...
        if name != tmp_name:
            ...
            # Several email addresses may be associated with a user
            email = item["email"]["value"]
            if email != tmp_email:
                tmp_email = email
                message += f" {email} "
...
##########################################################
# Queries for stations and trains                        #
##########################################################
...
def get_certificate(target_id: str, piece: str) -> Tuple[int, str]:
    ...
    return 2, message[:-2]  # Shouldnt end in new line char
...
def get_all(piece: str) -> Tuple[int, str]:
    ...
    emoji = ""
    if piece.lower() == "station":
        emoji = "🚉"
    else:
        emoji = "🚂"
    if len(body) == 1:
        return 2, f"{emoji} I found one {piece}: {body[0][{piece}]['value']}."
    if len(body) == 2:
...
def get_train_rejections(train_id: str, piece: str = None) -> Tuple[int, str]:
    ...
    message = f"⛔️ Train {train_id} rejections: "
...
##########################################################
# Queries not used anywhere yet - maybe irrelevant       #
##########################################################
...
def get_station_started_runs(station_id: str, piece: str = None) -> Tuple[int, str]:
    ...
    message = f"Last started run on {station_id} was by train: {sorted_values[0][1]}"
    return 2, message
...
def get_train_started_runs(train_id: str, piece: str = None) -> Tuple[int, str]:
    ...
    sorted_values = sort_time_values(response)
    message = f"Last started run of train {train_id} on station: {sorted_values[0][1]}"
    return 2, message
